...
const NewsCard = ({ item, t, lang, onPress }: { item: any, t: any, lang: any, onPress: () => void }) => (
    <div
        onClick={onPress}
        className="group relative bg-white rounded-[2rem] overflow-hidden shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:shadow-blue-900/10 transition-all duration-300 flex flex-col cursor-pointer h-full"
    >
        {/* Image Section - Full View */}
        <div className="relative aspect-[16/10] overflow-hidden bg-slate-50 border-b border-slate-100/50">
            <div className="absolute top-4 left-4 z-10">
                <Chip
                    size="sm"
                    className={item.type === 'promotion' ? "bg-brand-accent/90 text-white font-bold backdrop-blur-md border border-white/20 shadow-lg" : "bg-slate-900/90 text-white font-bold backdrop-blur-md border border-white/20 shadow-lg"}
                    variant="flat"
            ...
            <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-black/20 to-transparent opacity-60" />
        </div>
        ...
        <div className="p-6 md:p-8 flex-grow flex flex-col relative bg-white">
            ...
            <h3 className="text-xl md:text-2xl font-bold text-slate-900 leading-tight group-hover:text-brand-accent transition-colors line-clamp-2 mb-3">
                {item.title}
            </h3>
            <p className="text-slate-500 text-sm leading-relaxed line-clamp-2 font-light flex-grow">
                {item.short_description}
            </p>

            {/* Read More Link (Modern) */}
            <div className="flex items-center gap-2 text-slate-400 text-xs font-bold mt-6 group-hover:text-brand-accent transition-colors uppercase tracking-wider">
                <span>{t("อ่านเพิ่มเติม", "Read More")}</span>
                <div className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center group-hover:bg-brand-accent/10 transition-colors">
