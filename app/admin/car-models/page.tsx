...
export default function CarModelsPage() {
    ...
    const handleSave = async () => {
        try {
            ...
            if (res.ok) {
                toast.success(editingCar?.id ? "อัปเดตรุ่นรถสำเร็จ!" : "สร้างรุ่นรถใหม่สำเร็จ!");
                fetchCars();
                closeModal();
            } else {
                toast.error("ไม่สามารถบันทึกรุ่นรถได้");
            }
        } catch (error) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            toast.success("ลบรุ่นรถสำเร็จ");
            fetchCars();
            setIsDeleteModalOpen(false);
        } catch (error) {
            console.error("Error deleting car:", error);
            toast.error("ไม่สามารถลบรุ่นรถได้");
        }
    };
        ...
        <div className="mx-auto max-w-(--breakpoint-2xl) p-4 md:p-6 mb-6">
            ...
            <Table>
                ...
                <TableBody>
                    {loading ? (
                        <TableRow>
                            <TableCell colSpan={3} className="text-center py-8">กำลังโหลด...</TableCell>
                        </TableRow>
                    ) : cars.length === 0 ? (
                        <TableRow>
                            <TableCell colSpan={3} className="text-center py-8">ไม่พบรุ่นรถ</TableCell>
                        </TableRow>
                    ) : (
                        cars.map((item) => (
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal} className="max-w-3xl">
                <div className="mb-6">
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white">
                        {editingCar?.id ? "แก้ไขรุ่นรถ" : "เพิ่มรุ่นรถใหม่"}
                    </h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="col-span-1 md:col-span-2 flex flex-col gap-2">
                        <label className="text-sm font-medium text-gray-700 dark:text-gray-400">รูปภาพหลัก</label>
                    ...
                    <div className="col-span-1 md:col-span-2">
                        <label className="text-sm font-medium text-gray-700 dark:text-gray-400 mb-2 block">รุ่นย่อย</label>
                        <div className="space-y-3">
                            {formData.variants?.map((variant, index) => (
                                <div key={index} className="flex gap-3 items-center">
                                    <Input
                                        placeholder="ชื่อรุ่นย่อย (เช่น Premium)"
                                        value={variant.name}
                                        onChange={(e) => {
            ...
            <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}>
                ...
                <p className="text-gray-500 mb-6">
                    คุณแน่ใจหรือไม่ว่าต้องการลบรุ่นรถนี้? การดำเนินการนี้ไม่สามารถย้อนกลับได้
                </p>
